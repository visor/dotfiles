...
add_binds("normal", {
	...
	key({"Control"}, "k", "Move cursor to end of current line.", function (w)
		w:enter_cmd(":search ")
	end),
...
add_cmds({
	...
	cmd("s[earch]", "Поиск", function (w, a)
		w:navigate(w:search_open(a))
	end),

	cmd({"open.lc", "lc"}, "Открыть вкладку в домене .lc.", function (w, a)
		w:navigate("http://" .. a .. ".lc")
	end),

	cmd("extjs", "Открыть документацию по ExtJS .lc.", function (w)
		w:navigate("http://js.lc/extjs/ext-4.2.1.883/docs/index.html#!/api")
	end),
